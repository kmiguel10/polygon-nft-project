...
describe("nf-token-metadata", function () {
  ...
  it("correctly checks all the supported interfaces", async function () {
    expect(await nfToken.supportsInterface("0x80ac58cd")).to.equal(true); //erc721 base
    expect(await nfToken.supportsInterface("0x5b5e139f")).to.equal(true); //metadata
    expect(await nfToken.supportsInterface("0x780e9d63")).to.equal(false); //enumerable
  });
